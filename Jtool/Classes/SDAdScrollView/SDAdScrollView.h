//
//  TrendLiveViewController.m
//  BeautifulSow
//
//  Created by dandan on 16/4/11.
//  Copyright © 2016年 Mars. All rights reserved.
//

#import <UIKit/UIKit.h>

@interface SDAdScrollViewItem:NSObject

@property(nonatomic,copy)id image_url;

@property(nonatomic,copy)NSString * target_url;

@property(nonatomic,assign)BOOL completed;

+(instancetype)createWithImageUrl:(id)image_url andTargetUrl:(NSString*)target_url;

+(instancetype)createWithDictionary:(NSDictionary*)dictionary;

+(instancetype)createWithDictionary:(NSDictionary *)dictionary andKey:(NSArray<NSString*>*)key;

+(NSArray<SDAdScrollViewItem*>*)createListWithArray:(NSArray*)list;

+(NSArray<SDAdScrollViewItem*>*)createListWithArray:(NSArray *)list andKey:(NSArray<NSString*>*)key;

@end

/*
 *
 */

@interface SDAdScrollView : UIView
/*
 *显示广告的数据源，可以有三种类型 1:NSString，图片的网络地址，采用SDWebImage加载
 2:NSString，本地图片文件名
 *                            3:使用UIImage
 *  此属性赋值后会立即开启定时器，需要在下面两个属性设置完后再设置
 */
@property(nonatomic,copy)NSString * baseUrl;
@property (nonatomic, strong) NSArray <SDAdScrollViewItem*>*adList;
@property (nonatomic, assign, readonly) NSInteger currentPage;
@property (nonatomic, assign) NSTimeInterval animationDuration;//自动滚动动画时间，默认2s
@property (nonatomic, strong) UIPageControl *pageControl;
@property (nonatomic, strong) NSTimer *timer;
@property (nonatomic, strong) UIImage *placeHolderImage;
@property (nonatomic, assign) UIEdgeInsets scrollViewEdge;
@property(nonatomic,assign)UIViewContentMode  cmodel;
@property (nonatomic , copy) void (^tapActionBlock)(SDAdScrollView *adScrollView,SDAdScrollViewItem * item);
@property(nonatomic,copy)void(^imageDidLoad)(SDAdScrollView * adScrollview,NSInteger index);
@end
